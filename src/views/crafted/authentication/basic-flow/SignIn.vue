  ...
  <div class="w-lg-500px p-10">
    ...
    <VForm class="form w-100" id="kt_login_signin_form" @submit="onSubmitLogin" :validation-schema="login">
      ...
      <div class="fv-row mb-10">
        <!--begin::Label-->
        <label class="form-label fs-6 fw-bold text-dark">nik</label>
        <!--end::Label-->

        <!--begin::Input-->
        <Field tabindex="1" class="form-control form-control-lg form-control-solid" type="text" name="nik"
          autocomplete="off" placeholder="90019081" />
        <!--end::Input-->
...
export default defineComponent({
  ...
  setup() {
    ...
    const login = Yup.object().shape({
      nik: Yup.number().min(4).required().label("nik"),
    ...
    const onSubmitLogin = async (values: any) => {
      ...
      axios.post("http://10.10.0.223/backend-laravel-api/public/api/login", credentials).then((res) => {
        console.log(res);
        if (res.data.status === "Success") {
          Swal.fire({
            ...
            localStorage.setItem("teams", JSON.stringify(res.data.user.Authorization[0]));
